...
from scipy import linalg as mmmmm
...
class SingularSpectrumAnalysis():
    ...
    def nomalize(self):
        ...
        std = np.std(self.input_signal, axis=0)
        if(std != 0):
    ...
    def calculate_eigen_vectors_and_values(self):
        [self.eigne_values, self.eigne_vectors] = mmmmm.eig(self.covariance_matrix)
    ...
    def coss_corelation(self, x, y, max_lags):
        ...
        factor = np.sqrt(np.dot(x, x) * np.dot(y, y))
        if(factor != 0):
            c = c/factor
    ...
    def get_reconstructed_signal(self, start=0, end=1):
    ...
    def execute(self, number_of_pin_com=None):
        ...
        self.get_reconstructed_signal(end=number_of_pin_com)
